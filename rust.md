...
| Phase | 内容 | 依存 | 検証方法 |
|-------|------|------|----------|
| **1** | ビットボード + 着手生成 | なし | perft 一致 |
| **2** | 評価関数 | Phase 1 | 評価値一致 |
| **3** | ハッシュテーブル | Phase 1 | 単体テスト |
| **4** | 探索エンジン (シングルスレッド) | Phase 1-3 | fforum スコア一致 |
| **5** | 定石データベース | Phase 1, 4 | book.dat 読み書きラウンドトリップ |
| **6** | 並列探索 (YBWC) | Phase 4 | マルチスレッド fforum 一致 |
| **7** | ゲーム管理 + UI プロトコル | Phase 4-6 | 手動テスト |
| **8** | ベンチマーク + 最適化 | Phase 1-7 | NPS 比較 |
...
| eval.dat のバイナリ互換性 | 評価が不正 | C 版と同一局面でバイト単位の比較テスト |
| 連結リストの所有権問題 (`SquareList`) | 設計が複雑化 | インデックスベースの配列リストに変更 (性能差なし) |
| ハッシュテーブルの並行アクセス | データ競合 | `parking_lot::Mutex` のセット粒度ロック。C 版と同じ粒度 |
